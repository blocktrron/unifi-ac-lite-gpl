...
#define MARK_UBNT_SETMARK '6'
#define MARK_UBNT_ORMARK '7'
#define MARK_UBNT_ANDMARK '8'
#define MARK_UBNT_XORMARK '9'
static struct option opts[] =
{
	...
	{ "ubnt-mark-set"   , required_argument, 0, MARK_UBNT_SETMARK },
	{ "ubnt-mark-or"    , required_argument, 0, MARK_UBNT_ORMARK },
	{ "ubnt-mark-and"    , required_argument, 0, MARK_UBNT_ANDMARK },
	{ "ubnt-mark-xor"    , required_argument, 0, MARK_UBNT_XORMARK },
	{ 0 }
};
...
static void print_help()
{
	...
	" --ubnt-mark-set value    : Set ubnt_mark value\n"
	" --ubnt-mark-or value     : Or ubnt_mark with value (ubnt_mark |= value)\n"
	" --ubnt-mark-and value     : And ubnt_mark with value (ubnt_mark &= value)\n"
	" --ubnt-mark-xor value     : Xor ubnt_mark with value (ubnt_mark ^= value)\n"
...
#define OPT_MARK_UBNT_SETMARK  0x20
#define OPT_MARK_UBNT_ORMARK   0x40
#define OPT_MARK_UBNT_ANDMARK   0x80
#define OPT_MARK_UBNT_XORMARK   0x100
   ...
   struct ebt_entry_target **target)
{
	...
	case MARK_XORMARK:
		...
		markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | MARK_XOR_VALUE;
                break;
	case MARK_UBNT_SETMARK:
		ebt_check_option2(flags, OPT_MARK_UBNT_SETMARK);
		if (*flags & (OPT_MARK_UBNT_ORMARK|OPT_MARK_UBNT_ANDMARK|OPT_MARK_UBNT_XORMARK))
			ebt_print_error2("--ubnt-mark-set cannot be used together with specific --mark option");
		markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | MARK_UBNT_SET_VALUE;
				break;
	case MARK_UBNT_ORMARK:
		ebt_check_option2(flags, OPT_MARK_UBNT_ORMARK);
		if (*flags & (OPT_MARK_UBNT_SETMARK|OPT_MARK_UBNT_ANDMARK|OPT_MARK_UBNT_XORMARK))
			ebt_print_error2("--ubnt-mark-or cannot be used together with specific --mark option");
		markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | MARK_UBNT_OR_VALUE;
				break;
	case MARK_UBNT_ANDMARK:
		ebt_check_option2(flags, OPT_MARK_UBNT_ANDMARK);
		if (*flags & (OPT_MARK_UBNT_SETMARK|OPT_MARK_UBNT_ORMARK|OPT_MARK_UBNT_XORMARK))
			ebt_print_error2("--ubnt-mark-and cannot be used together with specific --mark option");
		markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | MARK_UBNT_AND_VALUE;
				break;
	case MARK_UBNT_XORMARK:
		ebt_check_option2(flags, OPT_MARK_UBNT_XORMARK);
		if (*flags & (OPT_MARK_UBNT_SETMARK|OPT_MARK_UBNT_ANDMARK|OPT_MARK_UBNT_ORMARK))
			ebt_print_error2("--ubnt-mark-xor cannot be used together with specific --mark option");
		markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | MARK_UBNT_XOR_VALUE;
				break;
	default:
		return 0;
	}
   ...
   const struct ebt_entry_target *target)
{
	...
	if (tmp == MARK_SET_VALUE)
		...
		printf("--mark-and");
	else if (tmp == MARK_UBNT_SET_VALUE)
		printf("--ubnt-mark-set");
	else if (tmp == MARK_UBNT_OR_VALUE)
		printf("--ubnt-mark-or");
	else if (tmp == MARK_UBNT_AND_VALUE)
		printf("--ubnt-mark-and");
	else if (tmp == MARK_UBNT_XOR_VALUE)
		printf("--ubnt-mark-xor");
	else
