...
#define DEFAULT_LOW_MEM 512 /* kB */

static unsigned int low_mem = DEFAULT_LOW_MEM;
static int enable_low_mem_check = 1;

#define OPT_FOREGROUND  (1 << 0)
#define OPT_STIMER      (1 << 1)
#define OPT_HTIMER      (1 << 2)
#define OPT_LOWMEM      (1 << 3)
...
static unsigned int _get_sys_total_mem(void)
{
    struct sysinfo info;
...
int watchdog_main(int argc, char **argv)
{
	...
	opts = getopt32(argv, "Ft:T:m:", &st_arg, &ht_arg, &low_mem_arg);

        if (opts & OPT_LOWMEM) {
            low_mem = xatou_range(low_mem_arg, 0, _get_sys_total_mem);
            if (low_mem == 0) {
                enable_low_mem_check = 0;
            }
        }
...
# endif
	...
	while (1) {
		...
		usleep(stimer_duration * 1000L);
                if (enable_low_mem_check && (watchdog_get_mem() < low_mem)) {
