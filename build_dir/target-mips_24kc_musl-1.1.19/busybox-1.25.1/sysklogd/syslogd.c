...
static int add_log_prefix(char *outbuf, int outlen, char *hostname, char *prefix, char *inbuf, int inlen)
{
	char *p, *tailp;
	int len, header_len, written, outleft;
	...
	inlen -= header_len;
    if (hostname) {
        len = snprintf(outbuf + written, outleft, "%.64s ", hostname);
        if (len < 0 || len >= outleft) {
            fprintf(stderr, "fail to add hostname\n");
        } else {
            written += len;
            outleft -= len;
        }
    ...
    if (prefix) {
        len = snprintf(outbuf + written, outleft, "%s: ", prefix);
        if (len < 0 || len >= outleft) {
            fprintf(stderr, "fail to add prefix\n");
        } else {
            written += len;
            outleft -= len;
        }
...
#define recvbuf (G.recvbuf)
 ...
 read_again:
		...
		for (item = G.remoteHosts; item != NULL; item = item->link) {
			...
			if (G.hostname || G.prefixRemoteLogStr) {
				len = add_log_prefix(G.printbuf, MAX_READ, G.hostname, G.prefixRemoteLogStr, recvbuf, sz + 1);
				sendbuf = G.printbuf;
			} else {
				len = sz + 1;
				sendbuf = recvbuf;
