...
int arping_main(int argc UNUSED_PARAM, char **argv)
{
	...
	packet = xmalloc(4096);
	/* change to use raw socket to receive arp reply */
	fd_rx = xsocket(PF_PACKET, SOCK_RAW, htons(ETH_P_ARP));
	while (1) {
		sigset_t sset, osset;
		...
		cc = recvfrom(fd_rx, packet, 4096, 0, (struct sockaddr *) &from, &alen);
		if (cc < 0) {
			bb_perror_msg("recvfrom");
			continue;
		}
        /* skip the ether header */
		memmove(packet, packet + sizeof(struct ethhdr), cc - sizeof(struct ethhdr));
		sigemptyset(&sset);
