...
int (*ubnt_loop_event)(struct net_device *dev, struct sk_buff *skb, int reason);
EXPORT_SYMBOL(ubnt_loop_event);

void br_fdb_update(struct net_bridge *br, struct sk_buff *skb, struct net_bridge_port *source,
		   const unsigned char *addr, u16 vid, bool added_by_user)
{
	...
	if (likely(fdb)) {
		...
		if (unlikely(fdb->is_local)) {
			if (net_ratelimit()) {
				if (ubnt_loop_event)
					ubnt_loop_event(source->dev, skb, 0);
...
void br_refresh_fdb_entry(struct net_device *dev, const char *addr)
{
	...
	rcu_read_lock();
	br_fdb_update(p->br, NULL, p, addr, 0, true);
	rcu_read_unlock();
}
	       ...
	       const unsigned char *addr, u16 nlh_flags, u16 vid)
{
	...
	if (ndm->ndm_flags & NTF_USE) {
		local_bh_disable();
		rcu_read_lock();
		br_fdb_update(p->br, NULL, p, addr, vid, true);
		rcu_read_unlock();
		local_bh_enable();
	} else {
