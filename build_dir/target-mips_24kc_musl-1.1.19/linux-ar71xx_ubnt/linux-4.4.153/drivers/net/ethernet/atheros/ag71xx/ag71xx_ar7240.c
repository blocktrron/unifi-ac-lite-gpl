...
#define AR7240_PORT_STATUS_BACK_PRESSURE	BIT(7)
#define AR7240_PORT_STATUS_LINK_UP	BIT(8)
#define AR7240_PORT_STATUS_LINK_AUTO	BIT(9)
#define AR7240_PORT_STATUS_LINK_PAUSE	BIT(10)
#define AR7240_PORT_STATUS_PHY_LINK_EN	BIT(12)

/* Phy Specific status fields */
#define AR7240_PHY_SPEC_STATUS_REG     (17)
#define AR7240_STATUS_LINK_MASK        0xC000
#define AR7240_STATUS_LINK_SHIFT       14
#define AR7240_STATUS_FULL_DUPLEX      0x2000
#define AR7240_STATUS_LINK_PASS        0x0400
#define AR7240_LATCH_LINK_PASS         0x0004
#define AR7240_STATUS_RESOVLED         0x0800
#define AR7240_STATUS_LINK_SPEED(_v)   ((_v & AR7240_STATUS_LINK_MASK) >> AR7240_STATUS_LINK_SHIFT)
...
int ar7240sw_phy_read(struct mii_bus *mii, unsigned phy_addr,
		      unsigned reg_addr)
{
	u32 t, val;
	int err;
...
static void ar7240sw_setup_port(struct ar7240sw *as, unsigned port, u8 portmask)
{
	...
	if (port == AR7240_PORT_CPU) {
		ar7240sw_reg_write(mii, AR7240_REG_PORT_STATUS(port),
				   ...
				   AR7240_PORT_STATUS_BACK_PRESSURE);
	} else {
		ar7240sw_reg_write(mii, AR7240_REG_PORT_STATUS(port),
				   AR7240_PORT_STATUS_LINK_AUTO |
				   AR7240_PORT_STATUS_PHY_LINK_EN |
				   AR7240_PORT_STATUS_BACK_PRESSURE);
	}
...
ar7240_hw_apply(struct switch_dev *dev)
{
	...
	if (as->vlan) {
		...
		/* vlan disabled:
		 * switch all ports, connect them to the cpu port */
		for (i = 0; i < as->swdev.ports; i++) {
		     ...
		     struct switch_port_link *link)
{
	...
	if (port == AR7240_PORT_CPU) {
		...
		status = ar7240sw_phy_read(mii, port, AR7240_PHY_SPEC_STATUS_REG);
		link->link = !!(status & AR7240_STATUS_LINK_PASS);
		link->duplex = !!(status & AR7240_STATUS_FULL_DUPLEX);
		switch(AR7240_STATUS_LINK_SPEED(status)) {
		case 0:
			link->speed = SWITCH_PORT_SPEED_10;
			break;
		case 1:
...
static void link_function(struct work_struct *work) {
	...
	int i;
	int global_status = 0;
	static int status[AR7240_NUM_PHYS] = {};
	static int an_timeout[AR7240_NUM_PHYS] = {};
	int aneg_restarted = 0;
	int link;
	...
	for (i = 0; i < AR7240_NUM_PHYS; i++) {
		...
		if ((link <= 0) || (link == 0xffff)) {
			global_status = ag->link; // Don't allow misread to trigger link adjust
		} else if (link & BMSR_LSTATUS) {
			global_status = 1;
			status[i] = 1;
			an_timeout[i] = PHY_AN_TIMEOUT * 2;
			continue;
		} else {
			status[i] = 0;
		}
		if (an_timeout[i] > 0) {
			an_timeout[i] = an_timeout[i] - 1;
		}
	}

	spin_lock_irqsave(&ag->lock, flags);
	if (global_status != ag->link) {
		ag->link = global_status;
	...
	for (i = 0; i < AR7240_NUM_PHYS; i++) {
		if (!(mask & BIT(i)))
			continue;
		if ((status[i] == 0) && (an_timeout[i] == 0)) {
			ar7240sw_phy_write(ag->mii_bus, i, MII_ADVERTISE, ADVERTISE_ALL | 
					ADVERTISE_PAUSE_CAP | ADVERTISE_PAUSE_ASYM);
			ar7240sw_phy_write(ag->mii_bus, i, MII_BMCR, BMCR_ANENABLE | BMCR_ANRESTART);
			an_timeout[i] = PHY_AN_TIMEOUT * 2;
			aneg_restarted = 1;
		}
	}
	if (aneg_restarted) {
		schedule_delayed_work(&ag->link_work, 4 * HZ);
		return;
	}
