...
static void
ar8xxxx_send_phy_event(struct ar8xxx_priv *priv, int port)
{
	struct phymsg msg;
	struct net_device *dev = priv->phy->attached_dev;
	struct net *net = dev_net(dev);

	msg.port_idx = port;
	msg.status = priv->link_up[port] ? 1 : 0;
	ubnt_net_notify((void *)net, RTNLGRP_LINK, RTM_PHY, (void *)&msg, sizeof(msg));
	return;
}
