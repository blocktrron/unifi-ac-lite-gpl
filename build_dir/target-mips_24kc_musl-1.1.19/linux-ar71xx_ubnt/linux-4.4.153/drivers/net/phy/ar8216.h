...
#define AR8XXX_ARL_LIST_HASHSIZE	32
#define AR8XXX_ARL_LIST_HASH(addr)	(((const u8 *)(addr))[0] % AR8XXX_ARL_LIST_HASHSIZE)
#define AR8XXX_NUM_ARL_RECORDS		2048
#define AR8XXX_NUM_ARL_CACHE_RECORDS	4096
#define AR8XXX_MAX_ARL_RETURNED		100
...
struct ar8xxx_chip {
	...
	void (*get_arl_entry)(struct ar8xxx_priv *priv, struct arl_read_tmp_entry *a,
...
struct ar8xxx_priv {
	...
	bool port4_phy;
	char buf[2048];
	char arl_buf[AR8XXX_NUM_ARL_CACHE_RECORDS * 32 + 256];
	...
	struct delayed_work arl_read_work;
	int mib_next_port;
	u64 *mib_stats;
	struct arl_entry arl_table[AR8XXX_ARL_LIST_HASHSIZE];
	...
	int arl_age_time;
	int arl_cache_age_time;
	bool port_ingress_mode_override[AR8X16_MAX_PORTS];
	bool port_egress_mode_override[AR8X16_MAX_PORTS];
	u8 port_ingress_mode_override_val[AR8X16_MAX_PORTS];
	u8 port_egress_mode_override_val[AR8X16_MAX_PORTS];
	u32 port_qos_egress_cir[AR8X16_MAX_PORTS];
	u32 port_qos_egress_eir[AR8X16_MAX_PORTS];
	u8 port_stp_state[AR8X16_MAX_PORTS];
	struct ar8xxx_phy_settings phy_settings[AR8XXX_NUM_PHYS];
	u32 vlan_detected_ports;
	bool acl_802_1d_redirect;
...
ar8xxx_sw_set_arl_age_time(struct switch_dev *dev,
			   ...
			   struct switch_val *val);
int
ar8xxx_sw_get_arl_cache_age_time(struct switch_dev *dev,
				 const struct switch_attr *attr,
				 struct switch_val *val);
int
ar8xxx_sw_set_arl_cache_age_time(struct switch_dev *dev,
				 const struct switch_attr *attr,
				 struct switch_val *val);
int
