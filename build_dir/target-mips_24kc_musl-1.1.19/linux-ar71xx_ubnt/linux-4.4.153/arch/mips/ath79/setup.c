...
unsigned long __init ubnt_get_mach_type(u16 sysid)
{
	switch(sysid)
	{
#define XX(sysid, ubntname, is_usb, is_switch, mach_type) \
		case sysid: \
			return mach_type;
#include <ubnt_sysid_to_mach.h>
#undef XX
		default:
			break;
	}
	return ATH79_MACH_GENERIC_OF;
}
...
static void __init ath79_generic_init(void)
{
	...
	if (id == 0xffff) {
		id = ntohs(*board_id_cal);
	}

	mips_machtype = ubnt_get_mach_type(id);

	if (mips_machtype == ATH79_MACH_GENERIC_OF)
		return;
	pr_info("calling mips_machine_setup on mips_machtype:%lx\n", mips_machtype);
