...
static struct flash_platform_data ubnt_unifi_ar7240_nor = {
	.type = "mx25l6405d",
	.name = "ar7240-nor0"
};

static u16 __init get_board_id(void)
{
	u16 *board_id = (u16 *)KSEG1ADDR(0x1fff000c);
	u16 *board_id_cal = (u16 *)KSEG1ADDR(0x1fff1016);
	u16 id = ntohs(*board_id);
	if (id == 0xffff) {
		return ntohs(*board_id_cal);
	}
	return id;
}
...
static void __init ubnt_unifi_setup(void)
{
	...
	u8 *ee = (u8 *) KSEG1ADDR(0x1fff1000);
	u16 board_id = get_board_id();

	pr_info("%s:%d board_id:%x\n", __FUNCTION__, __LINE__, board_id);
	if (board_id == SYSTEMID_UAPINWALL) {
		ath79_register_m25p80(&ubnt_unifi_ar7240_nor);
		ath79_register_mdio(0, ~0);

		// Inwall wants to register eth 1 first.
		ath79_init_mac(ath79_eth1_data.mac_addr, mac, 0);
		ath79_register_eth(1);
		ath79_eth0_data.phy_if_mode = PHY_INTERFACE_MODE_MII;
		ath79_eth0_data.speed       = SPEED_100;
		ath79_eth0_data.duplex      = DUPLEX_FULL;
		ubnt_unifi_leds_gpio[0].gpio = 16;
		ubnt_unifi_leds_gpio[0].default_trigger = "external0";
	} else if ((board_id == SYSTEMID_UAPLR) || (board_id == SYSTEMID_UAP)) {
		// for UAP v1 LR and UAP v1
		ath79_register_m25p80(&ubnt_unifi_ar7240_nor);
		ath79_register_mdio(0, ~UBNT_M_WAN_PHYMASK);
		ath79_eth0_data.phy_if_mode = PHY_INTERFACE_MODE_MII;
		ath79_eth0_data.speed       = SPEED_100;
		...
		ath79_register_m25p80(NULL);
		ath79_register_mdio(0, ~0);
		if (board_id == SYSTEMID_UAPWASP || board_id == SYSTEMID_UAPWASPLR) {
			ath79_eth0_data.phy_if_mode = PHY_INTERFACE_MODE_MII;
			ath79_eth0_data.speed       = SPEED_100;
			ath79_eth0_data.duplex      = DUPLEX_FULL;
			ubnt_unifi_leds_gpio[0].gpio = 14;
			ubnt_unifi_leds_gpio[0].default_trigger = "external0";
			ubnt_unifi_leds_gpio[1].gpio = 13;
			ubnt_unifi_leds_gpio[1].default_trigger = "external1";
		}
	}
	...
	// UAP v1 and UAP v1 LR have PCI radio only
	if ((board_id == SYSTEMID_UAPLR) || (board_id == SYSTEMID_UAP)) {
		ath79_init_mac(ath79_eth1_data.mac_addr, mac, 0);
...
static struct flash_platform_data ubnt_unifi_outdoor_flash_data = {
	.type = "mx2516405d",
	.name = "ar7240-nor0"
};
...
static void __init ubnt_unifi_outdoor_setup(void)
{
	...
	ath79_register_m25p80(&ubnt_unifi_outdoor_flash_data);
...
static struct flash_platform_data ubnt_unifi_outdoor_plus_flash_data = {
	.type = "mx25112805d",
	.name = "ar7240-nor0"
};
...
static void __init ubnt_unifi_outdoor_plus_setup(void)
{
	...
	ath79_register_m25p80(&ubnt_unifi_outdoor_plus_flash_data);
