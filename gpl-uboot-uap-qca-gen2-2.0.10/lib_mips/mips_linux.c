		     ...
		     ulong addr, ulong * len_ptr, int verify)
{
	...
	ulong data;
#if defined(CONFIG_AR7100) || defined(CONFIG_AR7240) || defined(CONFIG_MUSIC) || defined(CONFIG_ATHEROS)
...
#ifdef CONFIG_UBNT_DISABLE_GPS_SERIAL
    //printf ("going to clear AR7240_RESET_EXTERNAL... \n");
...
extern int g_logbuf_size;
extern int g_logbuf_offset;

const char uboot_version[] = U_BOOT_VERSION;
static void linux_params_init (ulong start, char *line)
{
	char *next, *quote, *argp, *mtds, *ubntbootid, *space;
	char memstr[64];
...
#define VIRIAN	"virian"
...
#if defined(CONFIG_AR9100) || defined(CONFIG_AR7240) || defined(CONFIG_MUSIC) || defined(CONFIG_ATHEROS)
	/* Add mem size to command line */
	if (memstr[0]) {
#if UBNT_PERSISTENT_LOG
		sprintf(memstr, "mem=%luK", (gd->ram_size - g_logbuf_size) >> 10);
#else
    ...
    space = strchr(uboot_version, ' ');
    next = space + 1;
    sprintf(memstr, "ubootver=%s", next);
    memcpy (argp, memstr, strlen(memstr)+1);
    linux_argv[linux_argc] = argp;
    linux_argc++;
    argp += strlen(memstr) + 1;
    ...
    {
        if (g_logbuf_offset) {
            sprintf(memstr, "ramoops.mem_address=0x%x", g_logbuf_offset);
            memcpy (argp, memstr, strlen(memstr)+1);
            linux_argv[linux_argc] = argp;
            linux_argc++;
            argp += strlen(memstr) + 1;
        ...
        if (g_logbuf_size) {
            sprintf(memstr, "ramoops.mem_size=%d", g_logbuf_size);
            memcpy (argp, memstr, strlen(memstr)+1);
            linux_argv[linux_argc] = argp;
            linux_argc++;
            argp += strlen(memstr) + 1;
