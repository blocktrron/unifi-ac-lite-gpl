...
#else
		if (!(strcmp(argv[argc-1], "-u"))) {
                        user_cmd = 1;
                }
#endif
                if (!(strcmp(argv[argc-1], "-e"))) {
			TftpServerUseDefinedIP = 1;
			printf("Using evnironment IP\n");
		}
		argc--;
	}
...
#else
        do_urescue = getenv("do_urescue");
        if (user_cmd || (do_urescue && !strncmp(do_urescue, "TRUE", 4))) {
	    return netboot_common(TFTP_SERVER, cmdtp, argc, argv);
        }
...
{
	const char *en = getenv("net_urescue");
	int retval;

	if (!en || strcmp(en, "TRUE"))
		return 0;

	retval = RecoveryInit();
	if (retval)
		return retval;
	return NetLoop(RECOVERY);
}

#if 0
int do_net_urescue (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	setenv("net_urescue", "TRUE");
