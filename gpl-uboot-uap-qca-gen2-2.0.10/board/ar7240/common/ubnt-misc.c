...
#define pll_clr(_mask) do { \
	ar7240_reg_rmw_clear(AR7240_CPU_PLL_CONFIG, _mask); \
} while (0)

#define clk_clr(_mask) do { \
	ar7240_reg_rmw_clear(AR7240_CPU_CLOCK_CONTROL, _mask); \
} while (0)

#define pll_set(_mask, _val) do { \
	ar7240_reg_wr_nf(AR7240_CPU_PLL_CONFIG, \
			(ar7240_reg_rd(AR7240_CPU_PLL_CONFIG) & ~(_mask)) | _val); \
} while (0)

#define clk_set(_mask, _val) do { \
	ar7240_reg_wr_nf(AR7240_CPU_CLOCK_CONTROL, \
			(ar7240_reg_rd(AR7240_CPU_CLOCK_CONTROL) & ~(_mask)) | _val); \
} while (0)
...
pll_modify(uint32_t val) {
	...
	/* wait for pll update */
	do {
		x = (ar7240_reg_rd(AR7240_CPU_PLL_CONFIG) & PLL_CONFIG_PLL_UPDATE_MASK) >> PLL_CONFIG_PLL_UPDATE_SHIFT;
	} while (x);
...
do_pll(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[]) {
	...
	pll = ar7240_reg_rd(AR7240_CPU_PLL_CONFIG) & PLL_MASK;
	clk_ctrl = ar7240_reg_rd(AR7240_CPU_CLOCK_CONTROL);
...
#define _GPIO_CR    AR7240_GPIO_OE
#define _GPIO_DO    AR7240_GPIO_OUT
#define _GPIO_DI    AR7240_GPIO_IN
...
gpio_set(unsigned char* gpios, int count) {
	...
	reg_dir = ar7240_reg_rd(_GPIO_CR);
	reg_out = ar7240_reg_rd(_GPIO_DO);
	...
	ar7240_reg_wr(_GPIO_CR, reg_dir);
	reg_dir = ar7240_reg_rd(_GPIO_CR); /* flush */

	ar7240_reg_wr(_GPIO_DO, reg_out);
	reg_out = ar7240_reg_rd(_GPIO_DO); /* flush */
...
gpio_bitbang_re(u8 clk_pin, u8 data_pin, u8 bitcount, u32 value) {
	...
	value <<= (32 - bitcount);
	ar7240_reg_wr_nf(AR7240_GPIO_CLEAR, clk);

	while (bitcount) {
		if (value & (1 << 31))
			ar7240_reg_wr_nf(AR7240_GPIO_SET, out);
		else
			ar7240_reg_wr_nf(AR7240_GPIO_CLEAR, out);

		udelay(1);
		ar7240_reg_wr_nf(AR7240_GPIO_SET, clk);
		udelay(1);
		value <<= 1;
		bitcount--;
		ar7240_reg_wr_nf(AR7240_GPIO_CLEAR, clk);
...
unifi_set_led(int pattern)
{
    ...
    reg_out = ar7240_reg_rd(AR7240_GPIO_OUT) & (~((1 << led1) | (1 << led2)));
    if (invert) {
        switch (pattern) {
            case 0:
                ar7240_reg_wr(AR7240_GPIO_OUT, reg_out | ((1 << led1) | (1 << led2)));
                break;
            case 1:
                ar7240_reg_wr(AR7240_GPIO_OUT, reg_out | (1 << led2));
                break;
            case 2:
                ar7240_reg_wr(AR7240_GPIO_OUT, reg_out | (1 << led1));
                break;
            case 3:
                ar7240_reg_wr(AR7240_GPIO_OUT, reg_out);
                break;
            default:
                break;
        }
...
gpio_input_get(u8 num) {
	...
	reg_in = ar7240_reg_rd(AR7240_GPIO_IN); /* flush */
...
do_gpio(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[]) {
	...
	printf("GPIO dir=0x%08x; out=0x%08x\n",
			ar7240_reg_rd(_GPIO_CR), ar7240_reg_rd(_GPIO_DO));
...
unifi_init_gpio(void)
{
    if (is_ar9344() || is_ar9341()) {
        ar7240_reg_rmw_clear(AR7240_GPIO_OE, (1 << 12));
        ar7240_reg_rmw_clear(AR7240_GPIO_OE, (1 << 13));
        ar7240_reg_wr(AR7240_GPIO_OUT, ar7240_reg_rd(AR7240_GPIO_OUT) & (~((1 << 12) | (1 << 13))));
    } else if (is_ar9342()) {
        ar7240_reg_rmw_clear(AR7240_GPIO_OE, (1 << 14));
        ar7240_reg_rmw_clear(AR7240_GPIO_OE, (1 << 13));
        ar7240_reg_wr(AR7240_GPIO_OUT, ar7240_reg_rd(AR7240_GPIO_OUT) & (~((1 << 14) | (1 << 13))));
    } else {
        if (gd->ram_size < 0x4000000) {
            ...
            ar7240_reg_rmw_set(AR7240_GPIO_OE, (1 << 1));
            ar7240_reg_rmw_set(AR7240_GPIO_OE, (1 << 11));
            ar7240_reg_wr(AR7240_GPIO_OUT, ar7240_reg_rd(AR7240_GPIO_OUT) & (~((1 << 1) | (1 << 11))));
        } else {
		    if (is_ar933x()) {
            	ar7240_reg_rmw_set(AR7240_GPIO_OE, (1 << 16));
                ar7240_reg_rmw_clear(AR7240_GPIO_FUNC, (1 << 6));
            }
            else {
                ar7240_reg_rmw_set(AR7240_GPIO_OE, (1 << 1));
            }
            ar7240_reg_rmw_set(AR7240_GPIO_OE, (1 << 0));
            ar7240_reg_wr(AR7240_GPIO_OUT, ar7240_reg_rd(AR7240_GPIO_OUT) & (~((1 << 1) | (1 << 0))));
        }
    }
    return 0;
}
