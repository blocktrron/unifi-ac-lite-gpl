...
pci_init_board (void)
{
...
#ifdef CONFIG_UBNT_NO_PCIE_WLAN
    PCI_INIT_RETURN;
#endif
...
#else
	uint32_t cmd;
    int try, pcie_wlan_found;

	if (is_drqfn() && !is_ar953x()) {
		/*
    ...
    pcie_wlan_found = 0;
    for (try = 0; (try < PCI_INIT_MAX_TRY) && (pcie_wlan_found == 0) ; try++) {
	...
	if (((ath_reg_rd(PCIE_RESET_ADDRESS)) & 0x1) != 0x0) {
		...
		//prmsg("*** %s Warning *** : PCIe WLAN Module not found !!!\n", __FUNCTION__);
    }

#endif

#ifdef PCIE2_APP_ADDRESS
	pcie_wlan_found = pci_rc2_init_board();
#endif

    //prmsg("*** log *** : try = %d, pcie_wlan_found = %d \n", try, pcie_wlan_found);
	 ...
	 * initialization code and return
	 */
	if (pcie_wlan_found == 0) {
		prmsg("*** Warning *** : PCIe WLAN Module not found !!!\n");
	} else {
        prmsg("PCIe WLAN Module found (tries: %d). \n", try);
    }
