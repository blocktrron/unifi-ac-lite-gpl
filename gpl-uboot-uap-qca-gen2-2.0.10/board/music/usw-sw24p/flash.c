...
#define SPI_MX25L128_ID         0xc22018
#define SPI_W25Q128FV_ID        0xef4018
#define SPI_EN25QH128_ID        0x1c7018
...
flash_get_geom (flash_info_t *flash_info)
{
    ...
    flash_info->flash_id  = FLASH_M25P32;
...
void msuic_spi_early_init(void)
{
    uint32_t spi_clock_div = 0;
    uint32_t reg_val = 0;
    uint32_t bus_freq = music_bus_freq_get();

    printf ("SPI0 early init\n");

    spi_clock_div = bus_freq / 50000000;
    spi_clock_div -= 1;
    spi_clock_div /= 2;
...
flash_init (void)
{
    ...
    if(flash_id == SPI_MX25L25635E_ID) {
        ...
        spi_init_info.spi_addr_select = NULL;
        spi_init_info.spi_addr_unselect = NULL;
    } else if (flash_id == SPI_MX25L128_ID) {
        spi_init_info.sector_support = 0;
        spi_init_info.valid = 1;
        spi_init_info.flash_id = flash_id;
        spi_init_info.page_size = 0x100;
        spi_init_info.sector_size = 0x1000;
        spi_init_info.block_size = 0x10000;
        ...
        spi_init_info.spi_addr_select = NULL;
        spi_init_info.spi_addr_unselect = NULL;
	} else if (flash_id == SPI_W25Q128FV_ID) {
        spi_init_info.sector_support = 0;
        spi_init_info.valid = 1;
        spi_init_info.flash_id = flash_id;
        spi_init_info.page_size = 0x100;
        ...
        spi_init_info.spi_addr_select = NULL;
        spi_init_info.spi_addr_unselect = NULL;
	} else if (flash_id == SPI_EN25QH128_ID) {
        spi_init_info.sector_support = 0;
        spi_init_info.valid = 1;
        spi_init_info.flash_id = flash_id;
        spi_init_info.page_size = 0x100;
