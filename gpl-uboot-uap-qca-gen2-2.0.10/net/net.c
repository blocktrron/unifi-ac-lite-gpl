...
{
	IP_t *ip = pkt - IP_HDR_SIZE;
	recovery_t *rec = pkt;
	char s_ipaddr[24];
	uint8_t *ipaddr, *netmask;

	if (ip->ip_p != IPPROTO_UDP || ip->udp_dst != htons(RECOVERY_PORT))
		return;
	...
	ipaddr[1] = (ipaddr[1] & netmask[1]) | (NetOurEther[3] & ~netmask[1]);
	ipaddr[2] = (ipaddr[2] & netmask[2]) | (NetOurEther[4] & ~netmask[2]);
	ipaddr[3] = (ipaddr[3] & netmask[3]) | (NetOurEther[5] & ~netmask[3]);
	...
	setenv("do_net_urescue", "TRUE");
	setenv("urescue_ip", s_ipaddr);
	TftpServerUseDefinedIP = 1;
}

#define NET_URESCUE_TIMEOUT_DEFAULT 3
#define NET_URESCUE_TIMEOUT_MAX 30

static void
RecoveryStart (void)
{
	char *timeout_s = getenv("net_urescue_timeout");
	uint32_t timeout = timeout_s ? simple_strtoul(timeout_s, NULL, 10) : NET_URESCUE_TIMEOUT_DEFAULT;
	if (timeout > NET_URESCUE_TIMEOUT_MAX)
		timeout = NET_URESCUE_TIMEOUT_MAX;

	printf("net_urescue: timeout=%d\n", timeout);
	timeout *= CFG_HZ;
	NetSetTimeout (timeout, RecoveryTimeout);
...
void
PingReply(volatile uchar * inpkt, int len)
{
    Ethernet_t *et;
    IP_t	*ip;
    ARP_t	*arp;
    ...
    case PROT_IP:
        ...
        if (ip->ip_p == IPPROTO_ICMP) {
            ...
            case ICMP_ECHO_REQUEST:
                ...
                NetCopyIP((void*)&ip->ip_dst, &desp_ip->ip_src);
                NetCopyIP((void*)&ip->ip_src, &NetOurIP); /* already in network byte order */
                ip->ip_sum   = ~NetCksum((uchar *)ip, IP_HDR_SIZE_NO_UDP / 2);
                ...
                s[1] = 0;
                s[1] = ~NetCksum((uchar *)s, 8/2 + (len -sizeof(IP_t))/2);

                eth_send((uchar *)et, (pkt - (uchar*)et) + len);
                break;
