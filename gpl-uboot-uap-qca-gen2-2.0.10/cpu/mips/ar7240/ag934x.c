...
#if defined(CONFIG_AR7242_S16_PHY) || defined(CONFIG_ATHRS17_PHY) || defined(CONFIG_ATHR8035_PHY)
    if (is_wasp() && mac->mac_unit == 0) {
#ifdef  CONFIG_AR7242_S16_PHY
#ifdef  ET_DEBUG
        printf("WASP  ----> S16 PHY *\n");
#endif  /* ET_DEBUG */
#else   /* CONFIG_AR7242_S16_PHY */
#ifdef  CONFIG_ATHRS17_PHY
#ifdef  ET_DEBUG
        printf("WASP  ----> S17 PHY *\n");
#endif  /* ET_DEBUG */
#else   /* CONFIG_ATHRS17_PHY */
#ifdef  ET_DEBUG
        printf("WASP  ----> 8035 PHY *\n");
#endif  /* ET_DEBUG */
#endif  /* CONFIG_ATHRS17_PHY */
#endif  /* CONFIG_AR7242_S16_PHY */
...
#ifdef CONFIG_WASP_DETECT_PHY
    if (is_wasp() && !(is_ar9342() && athr8032_phy_probe(0))) {
        udelay(1000 * 3000);
    }
#endif
#ifdef  CONFIG_ATHRS17_PHY 
...
int ag7240_enet_initialize(bd_t * bis)
{
    ...
    for (i = 0;i < CFG_AG7240_NMACS;i++) {
        ...
        if (ag7240_macs[i]->mac_unit == 0) { /* WAN Phy */
            ...
            {
#ifdef CONFIG_WASP_DETECT_PHY
                if (is_wasp() && !(is_ar9342() && athr8032_phy_probe(ag7240_macs[i]->mac_unit))) {
                    athrs17_reg_init();
                }
#endif
#ifdef  CONFIG_ATHRS17_PHY
